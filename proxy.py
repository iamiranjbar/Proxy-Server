...
class Response(object):
	def __init__(self, response):
		...
		for x in range(0,len(lines)):
			parts = lines[x].split(' ')
			if x == 0 and len(parts) > 1:
				self.header['Status Code'] = parts[1]
			if len(parts) > 1:
				if parts[1].find(';') >= 0:
...
class ProxyServer(object):
	...
	def sendMail(self, msg):
		...
		mailFrom = "MAIL FROM:<amirranjbar1377@ut.ac.ir>\r\n"
		clientSocket.send(mailFrom.encode())
		recv2 = clientSocket.recv(1024)
		recv2 = recv2.decode()
		self.logger("After MAIL FROM command: "+recv2)
		rcptTo = "RCPT TO:<amirranjbar1377@ut.ac.ir>\r\n"
			...
			try:
				if not self.caching or not cacheRes:
					...
					if stringToTimeConverter(cacheTime) > datetime.now():
						...
						print "<<<<<<<"
						if 'Status Code' in response.header and response.header['Status Code'] == '304':
							conn.send(cacheRes)
